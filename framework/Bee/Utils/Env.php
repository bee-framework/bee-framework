...
class Env {
    ...
    public static function getBaseUrl() {
        return self::getProtocol() . "://" . self::getHost() . '/' . self::getBasePath() . self::getDispatcherPrefix();
    }

    /**
     * @return string
     *
     * todo: a different attempt at automatic dispatcher detection
     */
    public static function getDispatcherPrefix() {
        $requestUri = $_SERVER['REQUEST_URI'];
        $scriptName = $_SERVER['SCRIPT_NAME'];
        return substr($requestUri, 0, strlen($scriptName)) == $scriptName ? $scriptName : '';
    }
...
//			return substr($protocol, 0, $pos);
    ...
    public static function getHtmlBase() {
        ...
        $base .= '/';
        try {
            Assert::hasText(self::getBasePath());
    ...
    public static function getPathInfo() {
        ...
        if (is_null(self::$pathInfo)) {
            ...
            if (!Strings::hasText($pathInfo) && array_key_exists(self::PATH_INFO_REWRITE_QUERY_PARAM, $_GET)) {
